...
- `JWT`
  - `JWT`는 `토큰 기반 인증`을 사용하여, `REST API`에 대한 `인증` 및 `권한 부여`를 쉽게 구현할 수 있고 `Session`에 비해서 서버에 부담이 적다고 판단했습니다. (`AWS 프리티어`를
    사용하고 있어서 서버의 리소스를 고려함)
- `MySQL`
  - `MySQL`은 관련된 `다양한 레퍼런스`들이 많고, 지금까지 많은 곳에서 사용하면서 `입증된 신뢰성`이 있어서 선택했습니다.
- `Spring Rest Docs`
